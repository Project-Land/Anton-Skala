        ...
        <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800" x-data="{ show: false }">
            <form action="{{ route('tasks.store-description-type') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-6" x-data="imageViewer()">
                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ __('Sadržaj') }}</label>
                    <div class="border border-gray-200 dark:border-gray-500 rounded-lg p-4 grid grid-rows sm:grid-cols-2 gap-6">
                        <div class="grid grid-cols-2 gap-2 border-r-2">
                            <div>
                            ...
                            <template x-if="imageUrl">
                                <div class="shrink-0">
                                    <img class="h-32 w-32 object-cover rounded-sm" :src="imageUrl" alt="Current profile photo" />
                                </div>
                            </template>
                        </div>
                        <div>
                            <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300" for="file_input">{{ __('Tekst') }}</label>
                            <textarea name="text" id="text" cols="30" rows="3" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500">
                                {{ old('text') }}
                            </textarea>
                        </div>
                    </div>
                ...
                <div class="flex" x-show="show">
                    <img class="w-full sm:w-1/3 p-4 border border-gray-500 rounded-lg mt-2" src="/images/description.png" alt="uvod">
                </div>
            </form>
        </div>
    </div>
