<x-app-layout>
    ...
    <div class="container px-6 lg:px-32 mx-auto grid">
        ...
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('tasks.index', ['lesson_id' => $task->lesson_id]) }}" class="inline-flex items-center p-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 rounded-md active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-lime">
        ...
        <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
            <form action="{{ route('tasks.store-correct-answer-type') }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="hidden" name="lesson_id" value="{{ $task->lesson_id }}">
                ...
                <div class="mb-6" x-data="imageViewer()">
                    ...
                    <div class="border border-purple-600 rounded-lg p-4 grid grid-cols-3 gap-6">
                        <div>
                            ...
                            <input type="text" name="question_text" id="" value="{{ $content['question']->text }}"
                        ...
                        <div class="inline-flex border-l-2 border-r-2 px-6">
                            ...
                            <template x-if="!imageUrl">
                                ...
                                <div class="shrink-0">
                                    <img class="h-16 w-16 object-cover rounded-sm" src="{{ asset('/'.$content['question']->image) }}" alt="image" />
                                </div>
                                @endif
                            </template>
                            <template x-if="imageUrl">
                                <div class="shrink-0">
                                    <img class="h-16 w-16 object-cover rounded-sm" :src="imageUrl" alt="image" />
                                </div>
                            </template>
                        </div>
                        <div class="inline-flex px-6">
                            ...
                            <div class="shrink-0">
                                <audio controls class="h-16 w-32">
                                    <source src="{{ asset('/'.$content['question']->audio) }}" type="audio/mpeg">
                                </audio>
                            </div>
                            @endif
                        </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-rows-1 gap-8 border rounded-lg p-4">
                        ...
                        @foreach(json_decode($content['answers'], true) as $answer)
                        <!-- One block -->
                        <div class="border border-violet-300 rounded-lg p-4 grid grid-cols-12 gap-6" x-data="imageViewer()">
                            <div class="col-span-3">
                                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300" for="file_input">{{ __('Tekst') }}</label>
                                <input type="text" name="answer_text[]" id="" value="{{ $answer['text'] }}"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500">
                            </div>
                            <div class="inline-flex border-l-2 border-r-2 px-6 col-span-4">
                                <div>
                                ...
                                <template x-if="!imageUrl">
                                    @if($answer['image'])
                                    <div class="shrink-0">
                                        <img class="h-16 w-16 object-cover rounded-sm" src="{{ asset('/'.$answer['image']) }}" alt="Current profile photo" />
                                    </div>
                                    @endif
                                </template>
                                <template x-if="imageUrl">
                            ...
                            <div class="inline-flex px-6 border-r-2 col-span-4">
                                ...
                                <div class="shrink-0">
                                    <audio controls class="h-16 w-32">
                                        <source src="{{ asset('/'.$answer['audio']) }}" type="audio/mpeg">
                                    </audio>
                                </div>
                                @endif
                            </div>
                            <div class="col-span-1">
                                <label class="block mb-2 text-xs font-medium text-gray-900 dark:text-gray-300" for="file_input">{{ __('Tačan odgovor?') }}</label>
                                <select name="answer_correct[]" id="" class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-1 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500">
                                    <option value="0" {{ $answer['is_correct']==false ? "selected" :"" }}>{{ __('Ne') }}</option>
                ...
                <button type="submit" class="w-1/2 md:w-1/5 mt-4 px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 rounded-md active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-lime">{{ __('Izmeni') }}
                </button>
            </form>
        </div>
    </div>
