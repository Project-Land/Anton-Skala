<x-app-layout>
    ...
    <div class="container px-6 mx-auto grid">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center mb-2">
            <h2 class="my-3 md:my-6 text-lg md:text-2xl font-semibold text-gray-700 dark:text-gray-200">{{ __('Škole') }}</h2>
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400 rounded-t-lg">
                    <tr>
                        <th scope="col" class="px-6 py-3">{{ __('Naziv') }}</th>
                        <th scope="col" class="px-6 py-3">{{ __('Država') }}</th>
                ...
                <tbody>
                    ...
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap">{{ $school->name }}</th>
                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap">
                            {{ $school->country == 'sr' ? 'Srbija': ($school->country == 'cro' ? 'Hrvatska':'Slovenija') }}
                        </th>
                        <td class="w-full px-6 py-4 text-right inline-flex justify-end space-x-4">
                            <a href="{{ route('schools.edit', $school) }}" class="font-medium text-blue-500 dark:text-blue-300 hover:underline">{{ __('Izmeni') }}</a>
                            {{-- <form action="{{ route('schools.destroy', $school) }}" method="POST" id="deleteSchool-{{ $school->id }}">
                                @csrf
                                @method('DELETE')
                                <button type="button" class="font-medium text-red-500 dark:text-red-300 hover:underline" onclick="deleteSchool({{ $school->id }})">{{ __('Obriši') }}</button>
                            </form> --}}
                        </td>
                    </tr>
                    @endforeach
                ...
                onOpen: function() {
                    modal.setContent(`
                        <div class="flex flex-col items-center space-y-4">
                            <h3 class="text-lg font-semibold">{{ __('Brisanje škole') }}</h3>
                            <p>{{ __('Ova akcija je nepovratna. Da li ste sigurni?') }}</p>
                        </div>
                    `);
