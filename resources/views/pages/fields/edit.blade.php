<x-app-layout>
    ...
    <x-slot name="title">| {{ __('Izmena oblasti') }}</x-slot>
    ...
    <div class="container px-6 lg:px-32 2xl:px-64 mx-auto grid">
        ...
        <div class="flex items-center justify-between">
            <h2 class="my-6 text-lg md:text-2xl font-semibold text-gray-700 dark:text-gray-200">
                {{ $field->name }} - {{ __('Izmena oblasti') }}
            </h2>
            <a href="{{ route('lessons.index', ['field_id' => $field->id]) }}" class="inline-flex items-center p-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 rounded-md active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-lime">
            ...
            <form action="{{ route('fields.update', $field->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div x-data="imageViewer()">
                    <div class="mb-6 inline-flex space-x-6 items-end">
                        ...
                        <template x-if="!imageUrl">
                            <div class="shrink-0">
                                <img class="h-32 w-32 object-cover rounded-sm border" id="img" src="{{ asset($field->image) }}" alt="Current profile photo" />
                            </div>
                        </template>
                        @endif
