<x-app-layout>
    ...
    <x-slot name="title">| {{ __('Izmena lekcije') }}</x-slot>
    ...
    <div class="container px-6 lg:px-32 2xl:px-64 mx-auto grid">
        ...
        <div class="flex items-center justify-between">
            <h2 class="my-6 text-lg md:text-2xl font-semibold text-gray-700 dark:text-gray-200">
                {{ $lesson->name }} - {{ __('Izmena lekcije') }}
            </h2>
            <a href="{{ route('tasks.index', ['lesson_id' => $lesson->id]) }}" class="inline-flex items-center p-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 rounded-md active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-lime">
        ...
        <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
            <form action="{{ route('lessons.update', $lesson->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div x-data="imageViewer()">
                    <div class="mb-6 inline-flex items-end space-x-6">
                        <div>
                        ...
                        <template x-if="!imageUrl">
                            <div class="shrink-0">
                                <img class="h-32 w-32 object-cover rounded-sm border" id="img" src="{{ asset($lesson->image) }}" alt="Current profile photo" />
                            </div>
                        </template>
                        @endif
                ...
                <div class="mb-6">
                    ...
                    <select name="lang" id="lang" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500">
                        <option value="sr_lat" {{ $lesson->lang == 'sr_lat' ? "selected":"" }}>{{ __('Latinica') }}</option>
                        <option value="sr_cir" {{ $lesson->lang == 'sr_cir' ? "selected":"" }}>{{ __('Ćirilica') }}</option>
                    </select>
                    @error('lang')
                    <span class="text-xs text-red-600 dark:text-red-400">{{ $message }}</span>
                    @enderror
